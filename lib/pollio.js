...
PollIO.prototype._handleAjaxCall = function(pollObject) {
	...
	if (this.pendingRequests[pollObject.identifier]) return;
	this.pendingRequests[pollObject.identifier] = true;
	
	pollObject.ajaxParams.success = function(results) {
		delete _this.pendingRequests[pollObject.identifier];
		pollObject.onResults(results, function() {
			delete _this.pollLookup[pollObject.identifier];
		});
	};

	pollObject.ajaxParams.error = function(jqXHR, textStatus, errorThrown) {
		delete _this.pendingRequests[pollObject.identifier];
		pollObject.onError(jqXHR, textStatus, errorThrown, function() {
			delete _this.pollLookup[pollObject.identifier];
		});
	};
